...
class LeagueJSRateLimiter { // TODO: unit tests
	 ...
	 * @param limitPer600
	 * @param allowBursts
	 * @param staticDataMonkeyPatch temporary fix for static-data rate limiting for damage control
	 * @return {{per10: {platformId: RateLimiter}, per600: {platformId: RateLimiter, limits: {per10, per600, allowBursts}}}}
	 */
	constructor(limitPer10, limitPer600, allowBursts, staticDataMonkeyPatch) {
		this.per10 = {};
		this.per600 = {};
		this.limits = {
			per10: limitPer10,
		...
		let higherResetInterval = staticDataMonkeyPatch ? 60 * 60 * 1000 : 600 * 1000;
		RegionAndPlatformUtil.getPlatformIds().forEach(platformId => {
			...
			this.per600[platformId] =
				new RateLimiter(limitPer600, higherResetInterval, allowBursts);
		});
	}
