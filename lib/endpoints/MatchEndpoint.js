...
class MatchEndpoint extends Endpoint {
	 ...
	 */
	async gettingListByAccount(puuid, platformIdOrRegion, options = {}) {
		...
		if ( numExpectedMatches > 100){
			...
			for (let i = 1; i <= numBatches; i++) {
				...
				const idsTemp = await this.gettingMatchIdsByPuuid(puuid, platformIdOrRegion, optionsForMatchIds)
				ids.push(...idsTemp);
			}
		} else {
		...
		const matches = await Bluebird.all(
			Bluebird.map(ids, (id)=>{
