...
class LeagueJS {
	...
	constructor(apiKey, options = {}) {
		...
		this.rateLimiter = new RiotRateLimiter({strategy: allowBursts ? RateLimiterStrategy.BURST : RateLimiterStrategy.SPREAD, debug: this.config.debug});
		...
		/** @deprecated removed from the API - will be removed here when runes reforged endpoints are available */
		...
		this.StaticData = new StaticDataEndpoint(this.config, this.rateLimiter);
	...
	static get GAME_CONSTANTS(){ return LeagueJS.util.GameConstantsUtil.GAME_CONSTANTS;}

	/** enables usage of DataDragonHelper (StaticDataReplacement) */
	setupDataDragonHelper(downloadPath, enableConsoleLogging){
		...
		/** temporary replacement for StaticDataEndpoint */
		this.DataDragonHelper = DataDragonHelper;
		if (enableConsoleLogging){
			DataDragonHelper.events.on(DataDragonHelper.eventIds.LOG_DEBUG, console.debug.bind(console));
			DataDragonHelper.events.on(DataDragonHelper.eventIds.LOG_INFO, console.info.bind(console));
			DataDragonHelper.events.on(DataDragonHelper.eventIds.LOG_ERROR, console.error.bind(console));
		}
		this.DataDragonHelper.storageRoot = downloadPath;
		return this.DataDragonHelper;
	}
