...
class LeagueJS {
	...
	constructor(apiKey, options = {}) {
		...
		this.rateLimiter = new RiotRateLimiter({strategy: allowBursts ? RateLimiterStrategy.BURST : RateLimiterStrategy.SPREAD});
	...
	setRateLimit(per10 = this.config.limits.per10, per600 = this.config.limits.per10, allowBursts = this.config.limits.allowBursts) {
		this.updateRateLimiter({strategy: (allowBursts ? RateLimiterStrategy.BURST : RateLimiterStrategy.SPREAD)});
	}
	...
	updateRateLimiter({allowBursts} = {}){
		this.util.ErrorUtil.throwIfNotBoolean(allowBursts, 'options.allowBursts');
