	...
	return versionPromise
		...
		.catch(err=>{
			...
			return DataDragonHelper.gettingVersions().then((versions) => {
				...
				if (skipLatestVersionFallback) {
					throw new Error('Could not receive data for ' + locale + ' ' + type + ' fallback: ' + indexFallbackVersion);
				} else {
		...
		return new Bluebird((resolve, reject) => {
			...
			request(uri, (err, httpResponse, body) => {
				...
				try {
					...
					emitError(new Error('Error while parsing response for locale: ' + locale + ' version: ' + version));
					return reject(new Error('Error while parsing response for locale: ' + locale + ' version: ' + version));
				}
