...
(function () {
	...
	apiUrl.league = {
		bySummoner: (platformId, summonerId) => `${apiUrl.host(platformId)}/lol/league/${API_VERSION}/leagues/by-summoner/${summonerId}`,
	...
	League.getMatchTimeLine = (matchId, platformId = platformIdDefault, callback = undefined) => {
		...
		return util.makeRequest(url, 'Error getting match timeline: ', null, callback);
	};
	...
	League.getMatchHistory = (accountId, {beginTime, endTime, beginIndex, endIndex, season, championIds, queue} = {}, platformId = platformIdDefault, callback = undefined) => {
		...
		queryString = queryString.substr(1); // remove first "&"
	...
	League.getChampions = function (freeToPlay, platformId = platformIdDefault, callback = undefined) {
        ...
        freetoPlayQuery = freetoPlayQuery.substr(1);
    ...
    League.Static.getChampionList = function({version, champData, dataById, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (champData) {
			...
			champDataArray.forEach(entry => {
				queryString += '&champListData=' + entry;
			});
		}
		if (dataById) {
	...
	League.Static.getChampionById = function(id, {version, champData, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (champData) {
			let champDataArray = [].concat(champData);
			champDataArray.forEach(entry => {
				queryString += '&champData=' + entry; // TODO: multiple data options are working differently now and have to be build as seen here
			});
		}
		if (locale) {
			queryString += '&locale=' + locale;
	...
	League.Static.getItemList = function({version, itemListData, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (itemListData) {
			...
			itemListDataArray.forEach(entry => {
				queryString += '&itemListData=' + entry; // TODO: multiple data options are working differently now and have to be build as seen here
			});
		}
		if (locale) {
			queryString += '&locale=' + locale;
	...
	League.Static.getItemById = function(id, {version, itemData, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (itemData) {
			...
			itemDataArray.forEach(entry => {
				queryString += '&itemData=' + entry; // TODO: multiple data options are working differently now and have to be build as seen here
			});
		}
		if (locale) {
			queryString += '&locale=' + locale;
	...
	League.Static.getMasteryList = function({version, masteryListData, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (masteryListData) {
			...
			masteryListDataArray.forEach(entry => {
				queryString += '&masteryListData=' + entry; // TODO: multiple data options are working differently now and have to be build as seen here
			});
		}
		if (locale) {
			queryString += '&locale=' + locale;
	...
	League.Static.getMasteryById = function(id, {version, masteryData, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (masteryData) {
			...
			masteryDataArray.forEach(entry => {
				queryString += '&masteryData=' + entry; // TODO: multiple data options are working differently now and have to be build as seen here
			});
		}
		if (locale) {
			queryString += '&locale=' + locale;
	...
	League.Static.getRuneList = function({version, runeListData, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (runeListData) {
			...
			runeListDataArray.forEach(entry => {
				queryString += '&runeListData=' + entry; // TODO: multiple data options are working differently now and have to be build as seen here
			});
		}
		if (locale) {
			queryString += '&locale=' + locale;
	...
	League.Static.getRuneById = function(id, {version, runeData, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (runeData) {
			...
			runeDataArray.forEach(entry => {
				queryString += '&runeData=' + entry; // TODO: multiple data options are working differently now and have to be build as seen here
			});
		}
		if (locale) {
			queryString += '&locale=' + locale;
	...
	League.Static.getSummonerSpellList = function({version, spellData, dataById, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (spellData) {
			...
			spellDataArray.forEach(entry => {
				queryString += '&spellListData=' + entry; // TODO: multiple data options are working differently now and have to be build as seen here
			});
		}
		if (dataById) {
			queryString += '&dataById=true';
	...
	League.Static.getSummonerSpellById = function(id, {version, spellData, locale} = {}, platformId = platformIdDefault, callback = undefined) {
		...
		if (spellData) {
			let spellDataArray = [].concat(spellData);
			spellDataArray.forEach(entry => {
				queryString += '&spellData=' + entry; // TODO: multiple data options are working differently now and have to be build as seen here
			});
		}
		if (locale) {
			queryString += '&locale=' + locale;
